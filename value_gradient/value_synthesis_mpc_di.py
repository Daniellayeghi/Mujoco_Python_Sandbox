...
def barrier_loss(x, x_u, x_l):
    ...
    loss = torch.maximum(x_low, zeros) * 10000 + torch.maximum(x_high, zeros) * 10000
...
def backup_loss(x: torch.Tensor, acc, alpha):
    ...
    l_running = state_loss_batch(x) + inv_dynamics_reg_batch(x, acc, alpha) + barrier_loss_batch(x, 10, -10).squeeze()*0
    value_final = nn_value_func(0, x_final.squeeze()).squeeze()
    value_initial = nn_value_func(0, x_initial.squeeze()).squeeze()
    loss = torch.square(value_final - value_initial*0 + l_running)
