...
class __MJAtrributes():
    def __init__(self, m: mujoco.MjModel, n_batch, n_bodies):
        self.m = m
        ...
        self._qfrc_t_batch = torch.zeros(
            (n_batch, self.m.nv), dtype=torch.float
        ).requires_grad_().to(device)

        self._dfinvdx_xd_t_batch = torch.zeros(
            (n_batch, (self._n_state + self.m.nv) * self.m.nv), dtype=torch.float
        ).requires_grad_().to(device)

        self._qfrc_t_bodies = torch.zeros(
            (n_bodies, self.m.nv), dtype=torch.float
        ).requires_grad_().to(device)
...
def torch_mj_set_attributes(m: mujoco.MjModel, n_batch, n_bodies):
    global _mj_attrs
    _mj_attrs = __MJAtrributes(m, n_batch, n_bodies)
...
class torch_mj_inv(Function):
    ...
    def forward(ctx, x_xd):
        ...
        for i in range(len(x_xd_np)):
            ...
            res[i, :] = np_to_tensor(_mj_attrs.d.qfrc_inverse, x_xd.device)
