...
if __name__ == "__main__":
    ...
    def loss_func(x: torch.Tensor):
        t, nsim, r, c = x.shape
        x_dist = x[:, :, :, :sim_params.nq].view(t, nsim, 1, sim_params.nq).clone()
        x_task = x[:, :, :, :sim_params.nv].view(t, nsim, 1, sim_params.nv).clone()
        loss_dist = torch.linalg.vector_norm(x_dist)
        loss_task = x_task @ Q @ x_task.mT
        return loss_task/loss_dist + loss_dist
